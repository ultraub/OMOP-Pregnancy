...
derive_ground_truth_outcome <- function(pregnancies_df, deliveries_df, id_linkage) {
  ...
  derived <- pregnancies_linked %>%
    ...
    mutate(
      ...
      # NOTE: Assume delta_para > 0 is LB unless explicit SB evidence
      # (LB is ~99% of births, so this is a reasonable default)
      gt_outcome_category = case_when(
        # Stillbirth: explicit evidence of fetal demise
        delta_para > 0 & !is.na(born_alive) & born_alive == 0 ~ "SB",
        delta_para > 0 & !is.na(living_status) & tolower(living_status) %in% c("stillborn", "fetal demise", "stillbirth") ~ "SB",
        # Live birth: para increased (default for births)
        delta_para > 0 ~ "LB",
        # Ectopic pregnancy
        delta_ectopic > 0 ~ "ECT",
...
# Debug: Check how many pregnancies have para count increases vs born_alive info
debug_outcomes <- gt_episodes %>%
  summarise(
    total = n(),
    has_born_alive = sum(!is.na(born_alive)),
    born_alive_1 = sum(born_alive == 1, na.rm = TRUE),
    born_alive_0 = sum(born_alive == 0, na.rm = TRUE),
    classified_lb = sum(gt_outcome_category == "LB"),
    classified_preg = sum(gt_outcome_category == "PREG"),
    classified_deliv = sum(gt_outcome_category == "DELIV", na.rm = TRUE)
  )
cat("\nOutcome derivation diagnostics:\n")
cat("- Total episodes:", debug_outcomes$total, "\n")
cat("- Has born_alive info:", debug_outcomes$has_born_alive, "\n")
cat("- born_alive = 1:", debug_outcomes$born_alive_1, "\n")
cat("- born_alive = 0:", debug_outcomes$born_alive_0, "\n")
cat("- Classified as LB:", debug_outcomes$classified_lb, "\n")
cat("- Classified as PREG:", debug_outcomes$classified_preg, "\n")
cat("- Classified as DELIV:", debug_outcomes$classified_deliv, "\n")
