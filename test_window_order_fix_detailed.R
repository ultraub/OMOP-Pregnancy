...
cat("Testing window_order fix with detailed error handling\n")
cat("=====================================================\n\n")
...
# Wrap in withCallingHandlers to get more detail
withCallingHandlers({
  ...
  cat("  Initial cohort created, now counting...\n")
  ...
  cat("  Calling final_visits function...\n")
  
  final_visits_df <- final_visits(initial_cohort, Matcho_outcome_limits, outcome_categories)
  
  cat("  final_visits returned, now collecting...\n")
  
  # Show the SQL being generated
  cat("\n  Generated SQL:\n")
  tryCatch({
    sql_query <- capture.output(show_query(final_visits_df))
    cat(paste("  ", head(sql_query, 20), collapse = "\n"), "\n")
  }, error = function(e) {
    cat("  Could not show query:", e$message, "\n")
  })
