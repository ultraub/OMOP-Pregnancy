...
match_episodes <- function(algo_episodes, gt_episodes, date_window_days = 30) {
  ...
  # Prepare algorithm data - assign relative order within each person
  algo <- algo_episodes %>%
    select(
      person_id,
    ...
    group_by(person_id) %>%
    arrange(algo_end) %>%
    mutate(algo_order = row_number()) %>%
    ungroup()

  # Prepare ground truth data - assign relative order within each person
  gt <- gt_episodes %>%
    select(
      person_id,
      gt_episode_num,
    ...
    group_by(person_id) %>%
    arrange(gt_end) %>%
    mutate(gt_order = row_number()) %>%
    ungroup()

  # Debug: Check person overlap before matching
  algo_persons <- unique(algo$person_id)
  gt_persons <- unique(gt$person_id)
  person_overlap <- intersect(algo_persons, gt_persons)
  cat("Persons in algorithm:", length(algo_persons), "\n")
  cat("Persons in ground truth:", length(gt_persons), "\n")
  cat("Person overlap for matching:", length(person_overlap), "\n")

  if (length(person_overlap) == 0) {
    warning("No person_id overlap between algorithm and ground truth!")
    return(list(
      matched = tibble(),
      unmatched_algo = algo,
      unmatched_gt = gt,
      stats = list(total_algo = nrow(algo), total_gt = nrow(gt), matched = 0,
                   match_rate_algo = 0, match_rate_gt = 0)
    ))
  }

  # MATCHING STRATEGIES (in order of preference):
  # 1. Match by relative temporal order within person (works with date shifting)
  # 2. Match by date proximity (if dates are not shifted)

  # Strategy 1: Match by relative order (1st episode to 1st, 2nd to 2nd, etc.)
  # This works even if dates are shifted by a constant offset per person
  matched_by_order <- algo %>%
    inner_join(gt, by = c("person_id", "algo_order" = "gt_order"))

  cat("Matched by relative temporal order:", nrow(matched_by_order), "\n")

  # Strategy 2: Match by date proximity (original method)
  matched_by_date <- algo %>%
  ...
  date_matches_count <- sum(matched_by_date$is_matched, na.rm = TRUE)
  cat("Matched by date proximity (±", date_window_days, " days):", date_matches_count, "\n")

  # Choose best strategy
  if (date_matches_count > 0) {
    # Use date matching if it works
    cat("\nUsing date-based matching\n")
    best_matches <- matched_by_date %>%
      filter(is_matched) %>%
      group_by(person_id, algo_episode_num) %>%
      slice_min(end_date_diff_abs, n = 1, with_ties = FALSE) %>%
      ungroup()
  } else if (nrow(matched_by_order) > 0) {
    # Fall back to relative order matching (handles date shifting)
    cat("\nUsing relative order matching (dates may be shifted in OMOP)\n")
    best_matches <- matched_by_order %>%
      mutate(end_date_diff = NA_real_, end_date_diff_abs = NA_real_)
  } else {
    best_matches <- tibble()
  }
